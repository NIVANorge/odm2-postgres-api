...
async def find_unit(conn: asyncpg.connection, unit: UnitsCreate, raise_if_none=False):
    row = await conn.fetchrow(
        f"SELECT * FROM units WHERE unitstypecv=$1 AND unitsabbreviation=$2", unit.unitstypecv, unit.unitsabbreviation
    )
...
async def create_new_controlled_vocabulary_item(
    conn: asyncpg.connection, controlled_vocabulary: ControlledVocabularyCreate
) -> ControlledVocabulary:
    ...
    row = await conn.fetchrow(
        make_sql_query(table_name, controlled_vocabulary_data), *controlled_vocabulary_data.values()
    )
    return schemas.ControlledVocabulary(**row)


async def create_or_parse_annotations(
    conn: asyncpg.connection, annotations: List[Union[schemas.AnnotationsCreate, int]]
):
    annotation_ids = []
...
async def create_sampling_feature(conn: asyncpg.connection, sampling_feature: schemas.SamplingFeaturesCreate):
    ...
    async with conn.transaction():
        ...
        for sampling_feature_id, relation_ship_type in sampling_feature.relatedsamplingfeatures:
            ...
            await insert_pydantic_object(
                conn, "relatedfeatures", related_sampling_feature_create, schemas.RelatedSamplingFeature
            )
...
async def create_sampling_feature_annotation(
    conn: asyncpg.connection, sampling_feature_annotation: schemas.SamplingFeatureAnnotationCreate
):
    ...
    await conn.fetchrow(
        "INSERT INTO samplingfeatureannotations (samplingfeatureid, annotationid) " "VALUES ($1, $2) returning * ",
        sampling_feature_annotation.samplingfeatureid,
...
async def create_result_data_quality(conn: asyncpg.connection, result_data_quality: schemas.ResultsDataQualityCreate):
    row = await conn.fetchrow(
        ...
        result_data_quality.resultid,
        result_data_quality.dataqualitycode,
    )
...
async def create_feature_action(conn: asyncpg.connection, feature_action: schemas.FeatureActionsCreate):
    if feature_action.samplingfeatureuuid and feature_action.samplingfeaturecode:
        row = await conn.fetchrow(
            ...
            feature_action.samplingfeatureuuid,
            feature_action.samplingfeaturecode,
        ...
        return schemas.FeatureActions(
            samplingfeatureuuid=feature_action.samplingfeatureuuid,
...
async def upsert_track_result(conn: asyncpg.connection, track_result: schemas.TrackResultsCreate):
    ...
    async with conn.transaction():
        ...
        if track_result.track_result_locations:
            location_records = (
                (rec[0], shapely.wkt.loads(f"POINT({rec[1]} {rec[2]})"), rec[3], track_result.samplingfeatureid)
        ...
        if track_result.track_result_values:
            value_records = (
                (rec[0], rec[1], rec[2], track_result.resultid) for rec in track_result.track_result_values
            )
            await conn.executemany(
...
async def upsert_measurement_result(conn: asyncpg.connection, measurement_result: schemas.MeasurementResultsCreate):
    async with conn.transaction():
        ...
        await conn.fetchrow(
            make_sql_query("measurementresults", measurementresults_data), *measurementresults_data.values()
        )
        await conn.fetchrow(
            make_sql_query("measurementresultvalues", measurementresultvalues_data),
...
async def upsert_categorical_result(conn: asyncpg.connection, categorical_result: schemas.CategoricalResultsCreate):
    async with conn.transaction():
        ...
        await conn.fetchrow(
            make_sql_query("categoricalresults", categoricalresults_data), *categoricalresults_data.values()
        )
        await conn.fetchrow(
            make_sql_query("categoricalresultvalues", categoricalresultvalues_data),
...
async def find_or_create_sampling_feature(conn, code: str, sf_type: str, wkt: str = None) -> SamplingFeatures:
    ...
    new_sf = SamplingFeaturesCreate(
        samplingfeatureuuid=uuid4(), samplingfeaturetypecv=sf_type, samplingfeaturecode=code, featuregeometrywkt=wkt
    )
