...
async def register_replicas(conn: asyncpg.connection, data: schemas.MsCreateReplicas) -> schemas.MsReplicas:
    ...
    async with conn.transaction():
        ...
        sampling_feature = schemas.SamplingFeaturesCreate(samplingfeatureuuid=uuid.uuid4(),
                                                          samplingfeaturecode=data.samplingfeaturecode,
                                                          samplingfeaturetypecv="Specimen",
                                                          relatedsamplingfeatures=[(
                                                              data.parent_samplingfeatureid,
                                                              "Is child of")],
                                                          annotations=[schemas.AnnotationsCreate(
        ...
        fractionate_sample_data = schemas.ActionsCreate(actiondescription="Fractionate water sample",
                                                        ...
                                                        affiliationid=1, isactionlead=True,
                                                        begindatetimeutcoffset=0)
        completed_fractionate_sample = await core_queries.do_action(conn, fractionate_sample_data)
        ran_mass_spec_data = schemas.ActionsCreate(actiondescription="Operated mass spectrometer",
                                                   actiontypecv="Specimen analysis",
                                                   methodcode="mass_spec:create_data",
                                                   begindatetime=action_time, sampling_features=[],
                                                   affiliationid=1, isactionlead=True, begindatetimeutcoffset=0)
        ...
        variables = await core_queries.find_row(conn, 'variables', 'variablecode', "mass_spec_00", schemas.Variables,
                                                raise_if_none=True)
        processinglevels = await core_queries.find_row(conn, 'processinglevels', 'processinglevelcode', "0",
                                                       schemas.ProcessingLevels, raise_if_none=True)
        resultannotation = schemas.AnnotationsCreate(annotationlink=data.resultannotationlink,
                                                     annotationtypecv="Result annotation", annotationcode="raw",
                                                     annotationtext="Check link for file location")
        result = schemas.ResultsCreate(samplingfeaturecode=sampling_feature.samplingfeaturecode, statuscv="Complete",
                                       variableid=variables.variableid, unitsid=units.unitsid,  # type: ignore
                                       processinglevelid=processinglevels.processinglevelid, valuecount=0,
                                       resulttypecv="Measurement", sampledmediumcv="Liquid aqueous",
                                       annotations=[resultannotation], resultdatetime=action_time,
                                       validdatetime=action_time, resultdatetimeutcoffset=0, validdatetimeutcoffset=0,
                                       actionid=completed_ran_mass_spec.actionid, resultuuid=uuid.uuid4())
        completed_result = await core_queries.create_result(conn, result)

    return schemas.MsReplicas(fractionate_sample=completed_fractionate_sample, ran_mass_spec=completed_ran_mass_spec,
...
async def register_sample(conn: asyncpg.connection, data: schemas.MsCreateSample) -> int:
    ...
    async with conn.transaction():
        ...
        if samplingfeatureid is None:
            ...
            ms_sample_data = schemas.ActionsCreate(actiondescription="Collected water sample",
                                                   actiontypecv="Specimen collection",
                                                   methodcode="mass_spec:collect_sample",
                                                   begindatetime=data.collection_time, isactionlead=True,
                                                   sampling_features=[sampling_feature], affiliationid=1,
                                                   begindatetimeutcoffset=0)
...
async def register_output(conn: asyncpg.connection, data: schemas.MsCreateOutput):
    ...
    async with conn.transaction():
        action_time = datetime.now()
        ran_ms_convert = schemas.ActionsCreate(actiontypecv="Derivation", methodcode=data.methodcode,
                                               begindatetime=action_time, sampling_features=[],
                                               affiliationid=1, isactionlead=True, begindatetimeutcoffset=0)
        ...
        units_create = schemas.UnitsCreate(unitstypecv="Dimensionless", unitsabbreviation="-", unitsname="")
        units = await core_queries.find_unit(conn, units_create, raise_if_none=True)
        variables = await core_queries.find_row(conn, 'variables', 'variablecode', data.variablecode,
                                                schemas.Variables, raise_if_none=True)
        processinglevels = await core_queries.find_row(conn, 'processinglevels', 'processinglevelcode', "0",
                                                       schemas.ProcessingLevels, raise_if_none=True)
        resultannotation = schemas.AnnotationsCreate(annotationlink=data.resultannotationlink,
        ...
        result = schemas.ResultsCreate(samplingfeaturecode=data.samplingfeaturecode, statuscv="Complete",
                                       variableid=variables.variableid, unitsid=units.unitsid,  # type: ignore
                                       processinglevelid=processinglevels.processinglevelid, valuecount=0,
                                       resulttypecv="Measurement", sampledmediumcv="Liquid aqueous",
                                       annotations=[resultannotation], resultdatetime=action_time,
                                       validdatetime=action_time, resultdatetimeutcoffset=0, validdatetimeutcoffset=0,
                                       actionid=completed_ran_ms_convert.actionid, resultuuid=uuid.uuid4())
