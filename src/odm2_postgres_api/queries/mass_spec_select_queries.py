...
async def find_result_annotationlink(conn: asyncpg.connection, data: schemas.MsResultAnnotationLinkQuery) -> \
    ...
    results = await conn.fetch(
        ...
        'where sf.samplingfeaturecode = $1 and a.annotationcode = $2 ', samplingfeaturecode, annotationcode)
...
async def get_samplingfeaturecodes_of_replicas(conn: asyncpg.connection, samplingfeaturecode: str) -> List[str]:
    result = await conn.fetch(
        ...
        'where r.relationshiptypecv = \'Is child of\' and sf2.samplingfeaturecode=$1 ', samplingfeaturecode)
...
async def register_replicas(conn: asyncpg.connection, data: schemas.MsCreateReplicas) -> schemas.MsReplicas:
    ...
    async with conn.transaction():
        ...
        sampling_feature = schemas.SamplingFeaturesCreate(
            ...
            annotations=[
                schemas.AnnotationsCreate(annotationjson=data.samplingfeatureannotationjson,
        ...
        variables = await core_queries.find_row(conn,
                                                'variables',
                                                'variablecode',
                                                "mass_spec_00",
                                                schemas.Variables,
                                                raise_if_none=True)
        processinglevels = await core_queries.find_row(conn,
                                                       'processinglevels',
                                                       'processinglevelcode',
                                                       "0",
    ...
    return schemas.MsReplicas(fractionate_sample=completed_fractionate_sample,
...
async def register_output(conn: asyncpg.connection, data: schemas.MsCreateOutput):
    ...
    async with conn.transaction():
        action_time = datetime.now()
        ran_ms_convert = schemas.ActionsCreate(actiontypecv="Derivation",
