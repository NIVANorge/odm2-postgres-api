...
class CVActionType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_actiontype'


class CVAggregationStatistic(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_aggregationstatistic'


class CVAnnotationType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_annotationtype'


class CVCensorCode(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_censorcode'


class CVDataQualityType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_dataqualitytype'


class CVDataSetType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_datasettypecv'


class CVDeploymentType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_deploymenttype'


class CVDirectiveType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_directivetype'


class CVElevationDatum(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_elevationdatum'


class CVEquipmentType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_equipmenttype'


class CVMediumType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_medium'


class CVMethodType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_methodtype'


class CVOrganizationType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_organizationtype'


class CVPropertyDataType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_propertydatatype'


class CVQualityCode(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_qualitycode'


class CVResultType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_resulttype'


class CVRelationshipType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_relationshiptype'


class CVSamplingFeatureGeoType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_samplingfeaturegeotype'


class CVSamplingFeatureType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_samplingfeaturetype'


class CVSpatialOffsetType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_spatialoffsettype'


class CVSpeciation(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_speciation'


class CVSpecimenType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_specimentype'


class CVSiteType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_sitetype'


class CVStatus(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_status'


class CVTaxonomicClassifierType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_taxonomicclassifiertype'


class CVUnitsType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_unitstype'


class CVVariableName(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_variablename'


class CVVariableType(db.Model, ODM2Mixin, CVMixin):
    __tablename__ = 'cv_variabletype'


class CVReferenceMaterialMedium(db.Model, ODM2Mixin, CVMixin):
...
# Core
# ################################################################################
class People(ODM2Mixin, db.Model):
    """
...
class Organizations(db.Model, ODM2Mixin):
    """
    A group of people.
    """
...
class Affiliations(db.Model, ODM2Mixin):
...
class Methods(db.Model, ODM2Mixin):
    """
...
class Actions(db.Model, ODM2Mixin):
    """
...
class ActionBy(db.Model, ODM2Mixin):
...
class SamplingFeatures(db.Model, ODM2Mixin):
    """
...
class FeatureActions(db.Model, ODM2Mixin):
    """
...
class DataSets(db.Model, ODM2Mixin):
    """
    Enables grouping of results into a larger dataset.
    """
    DataSetID = Column('datasetid', Integer, primary_key=True, nullable=False)

    # This has been changed to String to support multiple datadb.Model, ODM2Mixin uuid types
...
class ProcessingLevels(db.Model, ODM2Mixin):
    """
...
class RelatedActions(db.Model, ODM2Mixin):
    """
...
class TaxonomicClassifiers(db.Model, ODM2Mixin):
    """
...
class Units(db.Model, ODM2Mixin):
    """
    Units of measure.
    """
...
class Variables(db.Model, ODM2Mixin):
    """
    What was observed.
    """
...
class Results(db.Model, ODM2Mixin):
    """
...
class DataLoggerProgramFiles(db.Model, ODM2Mixin):
...
class DataLoggerFiles(db.Model, ODM2Mixin):
...
class EquipmentModels(db.Model, ODM2Mixin):
...
class InstrumentOutputVariables(db.Model, ODM2Mixin):
...
class DataLoggerFileColumns(db.Model, ODM2Mixin):
...
class Equipment(db.Model, ODM2Mixin):
...
class CalibrationReferenceEquipment(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    ActionID = Column('actionid', Integer, ForeignKey(Actions.ActionID), nullable=False)
...
class EquipmentActions(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class EquipmentUsed(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    ActionID = Column('actionid', Integer, ForeignKey(Actions.ActionID), nullable=False)
    EquipmentID = Column('equipmentid', Integer, ForeignKey(Equipment.EquipmentID), nullable=False)
...
class MaintenanceActions(db.Model, ODM2Mixin):
...
class RelatedEquipment(db.Model, ODM2Mixin):
...
class CalibrationActions(db.Model, ODM2Mixin):
    ActionID = Column('actionid', Integer, ForeignKey(Actions.ActionID), primary_key=True, nullable=False)
...
class Directives(db.Model, ODM2Mixin):
...
class ActionDirectives(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    ActionID = Column('actionid', ForeignKey(Actions.ActionID), nullable=False)
...
class SpecimenBatchPositions(db.Model, ODM2Mixin):
...
# Sampling Features
# ################################################################################
class SpatialReferences(db.Model, ODM2Mixin):
...
class Specimens(db.Model, ODM2Mixin):  # class Specimens(SamplingFeatures):
    SamplingFeatureID = Column('samplingfeatureid', ForeignKey(SamplingFeatures.SamplingFeatureID),
                               primary_key=True)
...
class SpatialOffsets(db.Model, ODM2Mixin):
...
class RelatedFeatures(db.Model, ODM2Mixin):
    RelationID = Column('relationid', Integer, primary_key=True, nullable=False)
    SamplingFeatureID = Column('samplingfeatureid', ForeignKey(SamplingFeatures.SamplingFeatureID),
                               nullable=False)
    RelationshipTypeCV = Column('relationshiptypecv', ForeignKey(CVRelationshipType.Name), nullable=False,
...
class SpecimenTaxonomicClassifiers(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
# Simulation
# ################################################################################
class Models(db.Model, ODM2Mixin):
...
class RelatedModels(db.Model, ODM2Mixin):
...
class Simulations(db.Model, ODM2Mixin):
...
class Citations(db.Model, ODM2Mixin):
...
# Annotations
# ################################################################################
class Annotations(db.Model, ODM2Mixin):
...
class ActionAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    ActionID = Column('actionid', ForeignKey(Actions.ActionID), nullable=False)
...
class EquipmentAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class MethodAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    MethodID = Column('methodid', ForeignKey(Methods.MethodID), nullable=False)
    AnnotationID = Column('annotationid', ForeignKey(Annotations.AnnotationID), nullable=False)
...
class ResultAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class SamplingFeatureAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    SamplingFeatureID = Column('samplingfeatureid', ForeignKey(SamplingFeatures.SamplingFeatureID),
                               nullable=False)
    AnnotationID = Column('annotationid', ForeignKey(Annotations.AnnotationID), nullable=False)
...
# Data Quality
# ################################################################################
class DataSetsResults(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class DataQuality(db.Model, ODM2Mixin):
...
class ReferenceMaterials(db.Model, ODM2Mixin):
...
class CalibrationStandards(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    ActionID = Column('actionid', Integer, ForeignKey(Actions.ActionID), nullable=False)
...
class ReferenceMaterialValues(db.Model, ODM2Mixin):
...
class ResultNormalizationValues(db.Model, ODM2Mixin):
...
class ResultsDataQuality(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    ResultID = Column('resultid', ForeignKey(Results.ResultID), nullable=False)
...
# ################################################################################
class ExtensionProperties(db.Model, ODM2Mixin):
...
class ActionExtensionPropertyValues(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    ActionID = Column('actionid', ForeignKey(Actions.ActionID), nullable=False)
...
class CitationExtensionPropertyValues(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    CitationID = Column('citationid', ForeignKey(Citations.CitationID), nullable=False)
    PropertyID = Column('propertyid', ForeignKey(ExtensionProperties.PropertyID), nullable=False)
    PropertyValue = Column('propertyvalue', String(255), nullable=False)
...
class MethodExtensionPropertyValues(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    MethodID = Column('methodid', ForeignKey(Methods.MethodID), nullable=False)
    PropertyID = Column('propertyid', ForeignKey(ExtensionProperties.PropertyID), nullable=False)
    PropertyValue = Column('propertyvalue', String(255), nullable=False)
...
class ResultExtensionPropertyValues(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    ResultID = Column('resultid', ForeignKey(Results.ResultID), nullable=False)
    PropertyID = Column('propertyid', ForeignKey(ExtensionProperties.PropertyID), nullable=False)
    PropertyValue = Column('propertyvalue', String(255), nullable=False)
...
class SamplingFeatureExtensionPropertyValues(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    SamplingFeatureID = Column('samplingfeatureid', ForeignKey(SamplingFeatures.SamplingFeatureID),
                               nullable=False)
    PropertyID = Column('propertyid', ForeignKey(ExtensionProperties.PropertyID), nullable=False)
...
class VariableExtensionPropertyValues(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    VariableID = Column('variableid', ForeignKey(Variables.VariableID), nullable=False)
    PropertyID = Column('propertyid', ForeignKey(ExtensionProperties.PropertyID), nullable=False)
    PropertyValue = Column('propertyvalue', String(255), nullable=False)
...
# ################################################################################
class ExternalIdentifierSystems(db.Model, ODM2Mixin):
...
class CitationExternalIdentifiers(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    CitationID = Column('citationid', ForeignKey(Citations.CitationID), nullable=False)
...
class MethodExternalIdentifiers(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    MethodID = Column('methodid', ForeignKey(Methods.MethodID), nullable=False)
    ExternalIdentifierSystemID = Column('externalidentifiersystemid',
                                        ForeignKey(ExternalIdentifierSystems.ExternalIdentifierSystemID),
...
class PersonExternalIdentifiers(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    PersonID = Column('personid', ForeignKey(People.PersonID), nullable=False)
    ExternalIdentifierSystemID = Column('externalidentifiersystemid',
                                        ForeignKey(ExternalIdentifierSystems.ExternalIdentifierSystemID),
...
class ReferenceMaterialExternalIdentifiers(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class SamplingFeatureExternalIdentifiers(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    SamplingFeatureID = Column('samplingfeatureid', ForeignKey(SamplingFeatures.SamplingFeatureID),
                               nullable=False)
    ExternalIdentifierSystemID = Column('externalidentifiersystemid',
...
class SpatialReferenceExternalIdentifiers(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    SpatialReferenceID = Column('spatialreferenceid', ForeignKey(SpatialReferences.SpatialReferenceID),
                                nullable=False)
    ExternalIdentifierSystemID = Column('externalidentifiersystemid',
...
class TaxonomicClassifierExternalIdentifiers(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    TaxonomicClassifierID = Column('taxonomicclassifierid',
                                   ForeignKey(TaxonomicClassifiers.TaxonomicClassifierID), nullable=False)
    ExternalIdentifierSystemID = Column('externalidentifiersystemid',
...
class VariableExternalIdentifiers(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    VariableID = Column('variableid', ForeignKey(Variables.VariableID), nullable=False)
    ExternalIdentifierSystemID = Column('externalidentifiersystemid',
                                        ForeignKey(ExternalIdentifierSystems.ExternalIdentifierSystemID),
...
class AuthorLists(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    CitationID = Column('citationid', ForeignKey(Citations.CitationID), nullable=False)
    PersonID = Column('personid', ForeignKey(People.PersonID), nullable=False)
...
class DataSetCitations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    DataSetID = Column('datasetid', ForeignKey(DataSets.DataSetID), nullable=False)
    RelationshipTypeCV = Column('relationshiptypecv', ForeignKey(CVRelationshipType.Name), nullable=False,
                                index=True)
...
class DerivationEquations(db.Model, ODM2Mixin):
...
class ResultDerivationEquations(db.Model, ODM2Mixin):
    ResultID = Column(u'resultid', ForeignKey(Results.ResultID), primary_key=True)
...
class MethodCitations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
    MethodID = Column('methodid', ForeignKey(Methods.MethodID), nullable=False)
    RelationshipTypeCV = Column('relationshiptypecv', ForeignKey(CVRelationshipType.Name), nullable=False,
                                index=True)
...
class RelatedAnnotations(db.Model, ODM2Mixin):
    RelationID = Column('relationid', Integer, primary_key=True, nullable=False)
    AnnotationID = Column('annotationid', ForeignKey(Annotations.AnnotationID), nullable=False)
    RelationshipTypeCV = Column('relationshiptypecv', ForeignKey(CVRelationshipType.Name), nullable=False,
                                index=True)
...
class RelatedCitations(db.Model, ODM2Mixin):
    RelationID = Column('relationid', Integer, primary_key=True, nullable=False)
    CitationID = Column('citationid', ForeignKey(Citations.CitationID), nullable=False)
    RelationshipTypeCV = Column('relationshiptypecv', ForeignKey(CVRelationshipType.Name), nullable=False,
                                index=True)
...
class RelatedDataSets(db.Model, ODM2Mixin):
    RelationID = Column('relationid', Integer, primary_key=True, nullable=False)
    DataSetID = Column('datasetid', ForeignKey(DataSets.DataSetID), nullable=False)
    RelationshipTypeCV = Column('relationshiptypecv', ForeignKey(CVRelationshipType.Name), nullable=False,
                                index=True)
...
class RelatedResults(db.Model, ODM2Mixin):
    RelationID = Column('relationid', Integer, primary_key=True, nullable=False)
    ResultID = Column('resultid', ForeignKey(Results.ResultID), nullable=False)
    RelationshipTypeCV = Column('relationshiptypecv', ForeignKey(CVRelationshipType.Name), nullable=False,
                                index=True)
...
# Results
# ################################################################################
class PointCoverageResults(db.Model, ODM2Mixin):
...
class ProfileResults(db.Model, ODM2Mixin):
    ResultID = Column('resultid', ForeignKey(Results.ResultID), primary_key=True)
...
class CategoricalResults(db.Model, ODM2Mixin):
    ResultID = Column('resultid', ForeignKey(Results.ResultID), primary_key=True)
    XLocation = Column('xlocation', Float(53))
...
class TransectResults(db.Model, ODM2Mixin):
    ResultID = Column('resultid', ForeignKey(Results.ResultID), primary_key=True)
    ZLocation = Column('zlocation', Float(53))
    ZLocationUnitsID = Column('zlocationunitsid', ForeignKey(Units.UnitsID))
    SpatialReferenceID = Column('spatialreferenceid', ForeignKey(SpatialReferences.SpatialReferenceID))
...
class SpectraResults(db.Model, ODM2Mixin):
    ResultID = Column('resultid', ForeignKey(Results.ResultID), primary_key=True)
    XLocation = Column('xlocation', Float(53))
    XLocationUnitsID = Column('xlocationunitsid', ForeignKey(Units.UnitsID))
    YLocation = Column('ylocation', Float(53))
...
class TimeSeriesResults(db.Model, ODM2Mixin):
    ResultID = Column('resultid', ForeignKey(Results.ResultID), primary_key=True)
    XLocation = Column('xlocation', Float(53))
    XLocationUnitsID = Column('xlocationunitsid', ForeignKey(Units.UnitsID))
    YLocation = Column('ylocation', Float(53))
...
class SectionResults(db.Model, ODM2Mixin):
    ResultID = Column('resultid', ForeignKey(Results.ResultID), primary_key=True)
    YLocation = Column('ylocation', Float(53))
    YLocationUnitsID = Column('ylocationunitsid', ForeignKey(Units.UnitsID))
    SpatialReferenceID = Column('spatialreferenceid', ForeignKey(SpatialReferences.SpatialReferenceID))
...
class TrajectoryResults(db.Model, ODM2Mixin):
    ResultID = Column('resultid', ForeignKey(Results.ResultID), primary_key=True)
    SpatialReferenceID = Column('spatialreferenceid', ForeignKey(SpatialReferences.SpatialReferenceID))
...
class MeasurementResults(db.Model, ODM2Mixin):
    ResultID = Column('resultid', ForeignKey(Results.ResultID), primary_key=True)
    XLocation = Column('xlocation', Float(53))
    XLocationUnitsID = Column('xlocationunitsid', ForeignKey(Units.UnitsID))
    YLocation = Column('ylocation', Float(53))
...
class CategoricalResultValues(db.Model, ODM2Mixin):
...
class MeasurementResultValues(db.Model, ODM2Mixin):
    ValueID = Column('valueid', BigIntegerType, primary_key=True)
...
class PointCoverageResultValues(db.Model, ODM2Mixin):
    ValueID = Column('valueid', BigIntegerType, primary_key=True)
...
class ProfileResultValues(db.Model, ODM2Mixin):
    ValueID = Column('valueid', BigIntegerType, primary_key=True)
...
class SectionResultValues(db.Model, ODM2Mixin):
    ValueID = Column('valueid', BigIntegerType, primary_key=True)
...
class SpectraResultValues(db.Model, ODM2Mixin):
    ValueID = Column('valueid', BigIntegerType, primary_key=True)
...
class TimeSeriesResultValues(db.Model, ODM2Mixin):
    ValueID = Column('valueid', BigIntegerType, primary_key=True)
...
class TrajectoryResultValues(db.Model, ODM2Mixin):
    ValueID = Column('valueid', BigIntegerType, primary_key=True)
...
class TransectResultValues(db.Model, ODM2Mixin):
    ValueID = Column('valueid', BigIntegerType, primary_key=True)
...
class CategoricalResultValueAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class MeasurementResultValueAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class PointCoverageResultValueAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class ProfileResultValueAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class SectionResultValueAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class SpectraResultValueAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class TimeSeriesResultValueAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class TrajectoryResultValueAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
...
class TransectResultValueAnnotations(db.Model, ODM2Mixin):
    BridgeID = Column('bridgeid', Integer, primary_key=True, nullable=False)
