...
from odm2_postgres_api.queries.mass_spec_select_queries import (
    ...
    register_replicas,
    register_output,
    get_samplingfeatureid_from_samplingfeaturecode,
    get_samplingfeaturecodes_of_replicas,
...
async def post_result_annotationlink(
    data: schemas.MsResultAnnotationLinkQuery,
...
async def samplingfeatureid_from_samplingfeaturecode(
    samplingfeaturecode: str, connection=Depends(api_pool_manager.get_conn)
) -> Optional[int]:
...
async def samplingfeaturecodes_of_replicas(
    samplingfeaturecode: str, connection=Depends(api_pool_manager.get_conn)
) -> List[str]:
...
async def samplingfeature_annotationjson(
    samplingfeaturecode: str, connection=Depends(api_pool_manager.get_conn)
) -> List[str]:
...
async def samplingfeaturecode_from_result_annotationlink(
    annotationlink: str, connection=Depends(api_pool_manager.get_conn)
) -> Optional[str]:
    return await get_samplingfeaturecode_from_result_annotationlink(connection, annotationlink)


@router.post("/register_replicas")
async def post_register_replicas(
    data: schemas.MsCreateReplicas, connection=Depends(api_pool_manager.get_conn)
) -> schemas.MsReplicas:
...
async def post_register_output(
    data: schemas.MsCreateOutput, connection=Depends(api_pool_manager.get_conn)
) -> schemas.MsOutput:
