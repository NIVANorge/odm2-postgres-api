...
def get_changed_containers(deployment_config):
    ...
    changed_containers = [
        name
        for name, container_description in deployment_config.items()
        if any(re.match(container_description["is_changed_regexp"], changed_file) for changed_file in changed_files)
    ]
...
def validate_config(container_config):
    correct_keys = [
        ...
        "kubectl_deployment_directories",
        "is_changed_regexp",
    ]
    ...
    for name, config in container_config.items():
        ...
        if type(kube_config) == dict:
            if not ("master" in kube_config and "not-master" in kube_config):
                raise ValueError(
                    f"If the kubectl_deployment_directories contains a dict you must specify at least "
    ...
    for name in container_config.keys():
        if type(container_config[name]["kubectl_deployment_directories"]) == dict:
            if os.environ.get("CIRCLE_BRANCH") == "master":
                container_config[name]["kubectl_deployment_directories"] = container_config[name][
                    "kubectl_deployment_directories"
                ]["master"]
            else:
                container_config[name]["kubectl_deployment_directories"] = container_config[name][
