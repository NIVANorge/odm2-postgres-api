...
from odm2_postgres_api.aquamonitor.aquamonitor_api_types import StationCargo, BegroingSampleCargo, MethodCargo, \
    TaxonomyCodeCargo, DomainCargo, TaxonomyCargo, BegroingObservationCargo, BegroingObservationCargoCreate
from odm2_postgres_api.aquamonitor.aquamonitor_client import post_begroing_observations
from odm2_postgres_api.schemas.schemas import BegroingObservations, Directive, SamplingFeatures, TaxonomicClassifier, \
    Methods, BegroingObservationValues
...
async def test_post_begroing_observations():
    async with AsyncClient(base_url="https://doesnotexist.niva.no") as client:
        ...
        station_cargo = StationCargo(Id=3,
                                     ProjectId=6,
                                     Type={},
                                     Code=station.samplingfeaturecode,
                                     Name=station.samplingfeaturename,
                                     Selected=False)
        mock_get_stations = respx.get(url=re.compile(r"^.*/Stations\?.*$"),
                                      status_code=200,
        ...
        mock_get_sample = respx.get(url=re.compile(r"^.*/query/begroing/samples\?.*$"),
                                    status_code=200,
        ...
        taxon = TaxonomyCodeCargo(Id=7587,
                                  Code="BRYOPHYT",
                                  Name="Bryophyta",
                                  Domain=DomainCargo(Id=9),
                                  Taxonomy=TaxonomyCargo(Id=9))
        mock_get_taxon = respx.get(url=re.compile(r"^.*/query/taxonomy/domains?.*$"),
                                   status_code=200,
                                   content=taxon.json())

        mock_post_observations = respx.post(url=re.compile(f"^.*/begroing/samples/{sample.Id}/observations?.*$"),
                                            status_code=200,
                                            content=BegroingObservationCargo(Id=555,
                                                                             Sample=sample,
                                                                             Method=method,
                                                                             Taxonomy=taxon,
                                                                             Value="x").json())

        begroing_observations = BegroingObservations(project=project,
                                                     date=sample_date,
                                                     station=station,
