...
async def test_post_begroing_observations():
    async with AsyncClient(base_url="https://doesnotexist.niva.no") as client:
        ...
        station_cargo = StationCargo(Id=3, ProjectId=6, Type={}, Code=station.samplingfeaturecode,
                                     Name=station.samplingfeaturename, Selected=False)
        ...
        mock_get_method_by_id = respx.get(
            url=re.compile(r"^.*/methods/\d*$"),
            status_code=200,
            content=method.json()
        )

        taxon = TaxonomyCodeCargo(Id=7587, Code="BRYOPHYT", Name="Bryophyta", Domain=DomainCargo(Id=9),
    ...
    for i, obs in enumerate(observations):
        ...
        obs_post_body = BegroingObservationCargoCreate(**json.loads(mock_post_observations.calls[i][0].stream.body))
